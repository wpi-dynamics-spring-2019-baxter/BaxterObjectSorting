...
Fruits::Fruits(NodeHandle& nh2D){
    ...
    sub2D = nh2D.subscribe < sensor_msgs::Image > ("/camera/rgb/image_raw", 1, &Fruits::subscriberCallback2D, this);
    my_serv2D = nh2D.advertiseService("convert2Dto3D", &Fruits::serviceCallback2D, this);
...
bool Fruits:: serviceCallback2D(fruitIdentifier::findCentroid::Request& req, fruitIdentifier::findCentroid::Request& res){
    res.centroids[0].header.frame_id = fruit;
    int number_of_fruits = fruit_centroids.size();
    res.centroids.resize(number_of_fruits);
    int count = 0;
    for (auto& itr: fruit_centroids){
        res.centroids[count].header.frame_id = fruit;
        res.centroids[count].pose.position.x = itr
    }
}
...
void Fruits::runAll(){
    ...
    switch (fruit){
        ...
        case 1:
            ...
            this->fruit_centroids.emplace_back(final_apple_centroids);
            //call get3dpoints with fruit_centroids and int
            break;
        case 2:
            cout<<"its a banana\n";
            ...
            this->fruit_centroids.emplace_back(final_banana_centroids);
            //call get3dpoints with fruit_centroids and int
            break;
        case 3:
            cout<<"its both!\n";
    ...
    cout<<"final size: "<<fruit_centroids.size()<<endl;
}
...
void Fruits::createAppleMask(){
    ...
    //cvNamedWindow("apple_mask1", CV_WINDOW_NORMAL);
    //cvNamedWindow("apple_mask2", CV_WINDOW_NORMAL);
    imshow("color", img);
    //imshow("apple_mask1", apple_mask1);
    //imshow("apple_mask2", apple_mask2);
    //waitKey(0);
}
...
void Fruits::createBananaMask(){
    ...
    cvNamedWindow("color", CV_WINDOW_NORMAL);
    //cvNamedWindow("banana_mask1", CV_WINDOW_NORMAL);
    //cvNamedWindow("banana_mask2", CV_WINDOW_NORMAL);
        imshow("color", img);
    //imshow("banana_mask1", banana_mask1);
    //imshow("banana_mask2", banana_mask2);
    //waitKey(0);
}
...
void Fruits::countPixels(){
    ...
    //cout<<"apple_count: "<<apple_count<<" banana_count: "<<banana_count<<endl;
}
...
void Fruits::morphOps(){
    ...
    dilate(b1, b1, Mat(),Point(-1,-1),3);
    //cout<<"done on b\n";
    //cvNamedWindow("b1", CV_WINDOW_NORMAL);
    //imshow("b1", b1);
...
vector<Moments> Fruits:: findAppleMoments(){
    ...
    //Canny( a1, a1, 255, 512, 3 );
    findContours( a1, apple_contours, apple_hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0) );

    //namedWindow( "Apple Canny", CV_WINDOW_NORMAL );
    //imshow( "Apple Canny", a1 );
    //waitKey(0);
    //cout<<"found apple moments\n";
    ...
    for(int i =0; i<a_mu.size(); i++)
        //cout<<a_mu[i].m00<<", "<<a_mu[i].m10<<endl;
        //cout<<endl;
        return a_mu;
}
...
vector<Moments> Fruits:: findBananaMoments(){
    ...
    // get the contours of bananas

    //Canny( b1, b1, 255, 512, 3 );
    //dilate(b1, b1, Mat(),Point(-1,-1),3);
    //erode(b1, b1, Mat(),Point(-1,-1),6);
    findContours( b1, banana_contours, banana_hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0) );

    //namedWindow( "Banana Canny", CV_WINDOW_NORMAL );
    //imshow( "Banana Canny", b1 );
    //waitKey(0);
...
vector<Point2f> Fruits:: findAppleCentroids(vector<Moments> a_mu){
    ...
    for( int i = 0; i<apple_contours.size(); i++)
    {
        ...
        if(a_mu[i].m00!=0 && apple_arclength>100){
            //cout<<"=================================\n";
            //cout<<"apple contour number: "<<i<<endl;
            //cout<<"apple contour size: "<<arcLength(Mat(apple_contours[i]),true)<<endl;
...
vector<Point2f> Fruits:: findBananaCentroids(vector<Moments> b_mu){
    ...
    for( int i = 0; i<banana_contours.size(); i++){
        ...
        if(b_mu[i].m00!=0 && banana_arclength>200){
            //cout<<"=================================\n";
            //cout<<"banana contour number: "<<i<<endl;
            //cout<<"banana contour size: "<<arcLength(Mat(banana_contours[i]),true)<<endl;
